...
await page.keyboard.up('KeyW');assert.ok(Math.hypot(moving.player.x,moving.player.y)>10);assert.equal(moving.metrics.drawCalls,8);
...
d=await diag(gpsPage);assert.ok(Math.abs(d.player.travelBearing)<1,`course ${d.player.travelBearing}`);assert.equal(d.metrics.drawCalls,8);assert.ok(d.metrics.fps>0);
...
assert.ok(mb.metrics.buildings>=20,`buildings ${mb.metrics.buildings}`);assert.equal(mb.metrics.drawCalls,8);assert.ok(mb.metrics.vertices<=90000);assert.ok(mb.metrics.fps===null||mb.metrics.fps>0);
