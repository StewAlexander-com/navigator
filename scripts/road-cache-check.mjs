...
try{
 ...
 const first=await page.evaluate(()=>window.navigatorDiagnostics());assert.ok(first.roadCache.bytes>0);assert.equal(first.street.name,'South Spring Street');assert.equal(first.street.kind,'STREET');assert.equal(first.metrics.drawCalls,11);
