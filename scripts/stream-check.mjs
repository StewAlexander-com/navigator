...
try{
 ...
 for(const [x,y] of [[0,0],[60,0],[120,0],[120,60],[60,100],[0,100],[-60,100],[-60,40],[0,0]]){
  ...
  assert.equal(d.area.live,false);assert.equal(d.metrics.drawCalls,7);assert.ok(d.metrics.stream.resident<=28);assert.ok(d.metrics.vertices<=90000);assert.ok(d.metrics.buildings<=160);assert.ok(d.metrics.roadVertices<=18000);
 }
